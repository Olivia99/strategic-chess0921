 ...
 * - Special: Invincible (cannot be captured) when in home rows
 * - Can capture enemy pieces
 */
export class GuardMovement implements PieceMovementRule {
  ...
  calculateAttacks(piece: Piece, board: (Piece | null)[][]): Position[] {
    // Guard attacks and moves are the same
  ...
  canCapture(piece: Piece, target: Position, board: (Piece | null)[][]): boolean {
    // Guard can capture if it can move there and there's an enemy piece
    return this.isValidMove(piece, piece.position, target, board) &&
           MovementValidator.hasEnemyPiece(target, piece.player, board);
  }

  /**
   * Check if Guard is invincible (cannot be captured)
   * Guards are invincible when in their home rows
   */
  isInvincible(piece: Piece): boolean {
    return MovementValidator.isInHomeRows(piece.position, piece.player);
  }
}